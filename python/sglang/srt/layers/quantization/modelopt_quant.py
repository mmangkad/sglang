...
def _apply_nvfp4_linear_torch(
    *,
    ...
    """Run NVFP4 GEMM through torch._scaled_mm with runtime input quantization."""
    assert torch_fp4_gemm_supported(), (
        "Torch NVFP4 backend requires torch._scaled_mm and "
        "torch.float4_e2m1fn_x2 support."
    )
    assert output_dtype == torch.bfloat16, (
        "Torch NVFP4 backend only supports bfloat16 outputs because fp16 can "
        "overflow before the external alpha correction is applied."
    )
...
class ModelOptFp4LinearMethod(LinearMethodBase):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        if fp4_backend.is_torch():
            if not torch_fp4_gemm_supported():
                raise ValueError(
                    "Torch NVFP4 backend requires torch._scaled_mm and "
                    "torch.float4_e2m1fn_x2 support."
                )
            layer.weight_torch = layer.weight.data.view(torch.float4_e2m1fn_x2)
            copy_or_rebind_param(
                layer,
    ...
    def apply(
        ...
        if fp4_backend.is_torch():
            return _apply_nvfp4_linear_torch(
                x=x,
                weight=getattr(layer, "weight_torch", layer.weight),
